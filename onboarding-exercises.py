...
def get_coords(stars):
    ts_fwd = np.linspace(0., 3., 2001) * u.Gyr
    x = stars.x(ts_fwd[-1]).value
    y = stars.y(ts_fwd[-1]).value
    z = stars.z(ts_fwd[-1]).value
    return x,y,z

def get_velocities(stars):
    ts_fwd = np.linspace(0., 3., 2001) * u.Gyr
    vx = stars.vx(ts_fwd[-1]).value
    vy = stars.vy(ts_fwd[-1]).value
    vz = stars.vz(ts_fwd[-1]).value
    return vx,vy,vz
...
def main(n=50000):
    ...
    np.save('results/o_lmc_x', o_lmc_plot.x(ts_bckwd).value)
    np.save('results/o_lmc_y', o_lmc_plot.x(ts_bckwd).value)
    np.save('results/o_lmc_z', o_lmc_plot.x(ts_bckwd).value)
    # unbiased sample
    ...
    stars_mw_radial = integrate_mw(ro,vo,stars_mw_radial,halo_potential,n) # was mw_interp
    r_mw_final_radial = (np.sqrt(stars_mw_radial.x(ts_fwd[-1])**2 + stars_mw_radial.y(ts_fwd[-1])**2 + stars_mw_radial.z(ts_fwd[-1])**2)).value
    r_mw_initial_radial = (np.sqrt(stars_mw_radial.x(ts_fwd[0])**2 + stars_mw_radial.y(ts_fwd[0])**2 + stars_mw_radial.z(ts_fwd[0])**2).value
    ...
    stars_full_radial = integrate_full(ro,vo,stars_full_radial,halo_potential,lmc_moving,nif,n) # was mw_interp
    r_full_final_radial = (np.sqrt(stars_full_radial.x(ts_fwd[-1])**2 + stars_full_radial.y(ts_fwd[-1])**2 + stars_full_radial.z(ts_fwd[-1])**2)).value
    r_full_initial_radial = (np.sqrt(stars_mw_radial.x(ts_fwd[0])**2 + stars_mw_radial.y(ts_fwd[0])**2 + stars_mw_radial.z(ts_fwd[0])**2)).value
    ...
    """
    config_path = os.path.expanduser("~/.galpyrc")
    print(config_path)
